...
* `resource`: Indicate the type of resource you want to target (e.g. vm, disk, ip, keypair, spot_instance_requests) or specify "all" to target all types of resources. Default: 'all'. AWS EBS volume deletion is opt-in and needs both `--cloud aws` and `--resource disk`; it is skipped under `--cloud all` and under `--resource all`.
