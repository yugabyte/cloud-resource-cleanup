...
* `resource`: Indicate the type of resource you want to target (e.g. vm, disk, ip, keypair, spot_instance_requests) or specify "all" to target all types of resources. Default: 'all'. AWS EBS volume deletion is opt-in and needs both `--cloud aws` and `--resource disk`. Any other combination that would hit the aws+disk path raises (it does not skip and continue), so `--cloud all` / `--resource all` cannot unlock other destructive AWS cleanups as a side effect.
...
* `custom_age_tag_key`: Define a custom tag key for the age threshold of resources on your cloud resource. This tag is ignored for `IPs` and `Keypairs` (e.g. value of the tag {'days': 3, 'hours': 12}).
...
* `detach_age`: Use this option to specify the detached age for filtering disks. This option only works for GCP and AWS disk deletions. **For AWS only**, `age` and `detach_age` accept only `days` and/or `hours`, and the duration must be greater than zero (that validation lives in `crc/aws/disk.py`; GCP does not apply the same check). For AWS, `detach_age` cannot exceed CloudWatch's 455-day metric retention; it is inferred from AWS/EBS CloudWatch metrics (`VolumeIdleTime`, `VolumeReadOps`, `VolumeWriteOps`), so the caller needs `cloudwatch:GetMetricData`. Those metrics are published only while a volume is attached to a *running* instance — the gate is not a last-detach timestamp and is blind to attachments on stopped instances. An AWS disk deletion requires `age` (measured from volume creation), `detach_age`, or both. CreateTime is always applied (using `detach_age` as the floor when `age` is omitted; a custom age tag is validated the same way and, in detach_age-only mode, the larger of the tag and `detach_age` wins). Untagged volumes are skipped unless `--notags` is set.
